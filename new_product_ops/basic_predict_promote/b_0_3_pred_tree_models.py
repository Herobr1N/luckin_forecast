...
# @created: 2023/10/9
# @author: jieqin.lin
# @file: projects/basic_predict_promote/b_0_3_pred_tree_models.py
...
def train_tree_models(target_date, model_label, data_label, goods_label, sel_goods_ls):
    """
    训练模型
    # =============================================================================
    # 功能：保存各个模型的预测数据
    ...
    # ----------------------
    log.debug(f"{model_label}_{data_label}_{goods_label}_货物数{len(sel_goods_ls)}_历史单日模型训练")
    ...
    # 划分测试集
    train_df, test_df, X_train, y_train, X_test, y_test = trainer.split_train_and_test(df=df_feature)

    # 训练
    model = trainer.select_tree_model(X_train=X_train, y_train=y_train)

    # 评估
    train_df['pred'] = model.predict(X_train).round(1)
    train_all, train_goods, train_wh_goods = trainer.evaluate_model_wh_goods(df=train_df,
                                                                             dt=target_date)
    # save 训练集准确率评估
    trainer.save_train_df(m_train_df=train_all, dt=target_date)
