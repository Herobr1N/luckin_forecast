...
# 依赖上游任务 bip3("model/basic_predict_promote", "stock_wh_goods_type_flg_theory_sale_cnt"), '2020-01-01', pred_minus1_day)
def consume_data(pred_minus1_day):
    ...
    # 列重命名
    df_stock_sell.rename(columns={
        "theory_sale_cnt": "y",
        "dt": "ds"},
        inplace=True)
    # 读取新店消耗
    df = df_stock_sell[
        (df_stock_sell["shop_type_flg"] == "new_shop")
        & (df_stock_sell["type_flg"] == "norm")
        ].reset_index(drop=True).copy()
    df = df.groupby(["ds", "wh_dept_id", "goods_id"]).agg({"y": "sum"}).reset_index()
...
class TreeModelTrainer:
    ...
    def model_prediction(self, start_date, window, goods_id, wh_dept_id, model, df):
        ...
        for dt in pd.date_range(start_date, day_30):
            ...
            # 特征重生成
            df_target = feature_generated(df_target)
            # 将生成好特征的最新一行来预测最新的一天
            next_day_feature = df_target.iloc[-1:].copy()
...
def main(pred_calculation_day=None):
    ...
    log.debug(f"execute date -- {pred_minus1_day}")
    # 读取数据
    df_stock_sell = consume_data(pred_minus1_day)
    log.debug(f"feature generated -- {pred_minus1_day}")
    # 新店特征
    df_xgb = feature_combined(df_stock_sell)
    # 新店特征落库
    log.debug(f"新店特征落库 -- {pred_minus1_day}")

    bip3_save_df2(
        df_xgb,
        table_folder_name='feature_engineering_wh_goods_newshop',
        bip_folder='model/basic_predict_promote',
        output_name=f"feature_engineering_wh_goods_newshop",
        folder_dt=pred_minus1_day
    )

    # 新店落表
    features = ['goods_id', "wh_dept_id", 'year', 'month', 'day', 'day_of_week', 'lag_1_mean', 'lag_7_mean',
                'lag_7_std', 'lag_7_min', 'lag_7_max', 'lag_14_mean',
    ...
    trainer = TreeModelTrainer(features, true_label="y", train_ratio=1, model_label="xgb")

    train_df, test_df, X_train, y_train, X_test, y_test = trainer.split_train_and_test(df_xgb)
    model = trainer.train_model(X_train, y_train)
    df_country = trainer.country_prediction(start_date=pred_minus1_day, window=30, model=model, df=df_xgb)

    log.debug(f"新店预测落库 -- {pred_minus1_day}")
    bip3_save_df2(
        df_country,
        table_folder_name='predict_wh_goods_newshop',
        bip_folder='model/basic_predict_promote',
        output_name=f"predict_wh_goods_newshop",
        folder_dt = pred_minus1_day
    )
    return None