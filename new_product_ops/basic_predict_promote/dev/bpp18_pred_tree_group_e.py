...
# @created: 2023/11/22
# @author: jieqin.lin
# @file: projects/basic_predict_promote/dev/bpp17_pred_tree_group_e.py
...
def train_and_evaluate_models(target_date, model_label, data_label, goods_label, window):
    ...
    sel_goods_ls = selector.get_group_e_goods_id()
    # ----------------------
    log.debug("历史单日模型训练")
    ...
    bip3_save_model(model,
                    ...
                    bip_folder='model/basic_predict_promote_group_e',
...
def recent_month_wh_goods_metric_mean(data_label, model_label, window):
    ...
    df_metric = read_api.read_dt_folder(
        bip3("model/basic_predict_promote_group_e", f"metric_month_wh_goods_{model_label}_wh_goods_pred_{data_label}"),
...
def recent_month_goods_metric_mean(data_label, model_label, window):
    ...
    # 最近3个月准确率均值
    df_metric = read_api.read_dt_folder(
        bip3("model/basic_predict_promote_group_e", f"metric_month_goods_{model_label}_wh_goods_pred_{data_label}"),
        start_dt, pred_minus1_day
    )
    df_metric['dt'] = pd.to_datetime(df_metric['dt'])
